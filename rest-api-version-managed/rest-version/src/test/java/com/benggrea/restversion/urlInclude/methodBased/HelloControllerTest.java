...
@DisplayName("API 버전을 메서드 별로 URI 포함하여 서비스를 관리한다")
    ...
    Stream<DynamicTest> apiUrlIncludeVersion() {
        return Stream.of(
                dynamicTest("API v1 - 기존 서비스", () -> {
                    ...
                    // when, then
                    mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/version")
                ...
                dynamicTest("API v2 - 새로운 서비스", () -> {
                    ...
                    // when, then
                    mockMvc.perform(MockMvcRequestBuilders.get("/api/v2/version")
                                    .accept(MediaType.APPLICATION_JSON)
                                    .contentType(MediaType.APPLICATION_JSON))
                            .andExpect(status().isOk())
                            .andExpect(content().bytes(expectedResponse.getBytes(StandardCharsets.UTF_8)))
